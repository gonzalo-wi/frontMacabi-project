  ...
  return (
    <Card className="rounded-2xl shadow-sm overflow-hidden">
            ...
            {pageItems.map((m) => {
              ...
              return (
                ...
                >
                  ...
                  <ActionIconButton
                    intent="delete"
                    label={`Quitar a ${displayName}`}
                    onClick={() => setRemoveTarget({ userId: m.user_id, name: displayName })}
                  >
      ...
      <ConfirmDialog
        ...
        title={removeTarget ? `¿Quitar a ${removeTarget.name}?` : ''}
        description={
          removeTarget
            ? `${removeTarget.name} dejará de figurar como miembro de este proyecto.`
            : undefined
        }
        confirmLabel="Quitar"
        destructive
        loading={remMem.isPending}
        onConfirm={() => {
          if (removeTarget) remMem.mutate(removeTarget.userId)
        }}
      />
    </Card>
  )
